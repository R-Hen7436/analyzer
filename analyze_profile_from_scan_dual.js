...
// Work laptop (same profiles dir for UVP and behavior; restore when using real p4work):
// const PROFILE_DIR =
...
// Local profiles for testing without work laptop:
const PROFILE_MK_PATH = path.join(__dirname, "dummy_profile.mk");
const BEHAVIOR_PROFILE_MK_PATH = path.join(__dirname, "mo2mmtsm.mk");
...
function buildNotes(kind, profileValue, occurrenceCount) {
    ...
    if (kind === "behavior") {
        if (profileValue === "TRUE") {
            return "Behavior found in ren_epc and paired UVP switch is TRUE in behavior profile";
        }

        if (profileValue === "FALSE") {
            return "Behavior found in ren_epc but paired UVP switch is FALSE in behavior profile";
        }

        return "Behavior found in ren_epc but paired UVP switch is missing from behavior profile";
    }
...
function buildRows(scanResult, uvpValues, behaviorValues) {
    const summaryRows = [];
    const detailRows = [];
    const impactRows = [];
...
function writeJsonReport(scanResult, uvpValues, behaviorValues, resultSwitches) {
    const jsonData = {
...
async function main() {
    ...
    const {
        ...
        impactRows,
        resultSwitches
    } = buildRows(scanResult, uvpValues, behaviorValues);

    await writeExcelReport(summaryRows, detailRows, impactRows);
    writeJsonReport(scanResult, uvpValues, behaviorValues, resultSwitches);
