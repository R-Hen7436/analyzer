...
| **Mapping log** | Audit trail: what matched, what was added, what did not match |
...
Think of it as **Find -> Judge -> Report**.
...
```text
  Code + profiles                Official Excel template
         ...
         v                                  |
   +-------------+                          |
   | 1. SCAN     |  Find switches in renEPC |
   +------+------+                          |
          |                                 |
          v                                 |
   +-------------+                          |
   | 2. ANALYZE  |  Compare to UVP +        |
   |             |  behavior profiles       |
   |             |  -> O / X / -            |
   +------+------+                          |
          |                                 |
   ...
   +---------------------------------------------+
   | 3. MAP                                      |
   |    Fill EMD Excel + write mapping log       |
   +---------------------------------------------+
```
...
> "For workspace *W* and UVP profile *P*, with behavior profile *B*, show me which renEPC switches pass or fail, which functions they hit, and update the change-point Excel."
...
| Hard to reproduce last week's check | Same command -> same folder layout |
...
| [`json_outputs_guide.md`](./json_outputs_guide.md) | How scan / analyze / mapping JSON helps |
