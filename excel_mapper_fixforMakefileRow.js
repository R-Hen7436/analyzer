...
function detectSection(bText, current) {
    ...
    if (text === "ùùUVP" || text === "UVP") {
        return SECTION.UVP;
    }

    if (text === "ùùVCP" || text === "VCP") {
        return SECTION.VCP;
    }

    if (
        text === "ùùBehavior Mode" ||
        text === "Behavior Mode" ||
        /^ùù?\s*Behavior\s*Mode$/i.test(text)
    ) {
    ...
    if (
        text === "Local Switch" ||
        text === "ùùLocal Switch" ||
        /^ùù?\s*Local\s+Switch$/i.test(text)
    ) {
        return SECTION.LOCAL_SWITCH;
    }

    // Separate model list under ùùPRD_SW ? out of scope (not in local_switch.json)
    if (text === "ùùPRD_SW" || text === "PRD_SW") {
 ...
 * Log entries recorded earlier still hold pre-shift emdRow values ù bump them
